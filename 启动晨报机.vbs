' =====================================================================
' Morning Brief launcher: start   (double-click to start in the background)
' =====================================================================
...
If "start" = "status" Then
  sh.Run "cmd.exe /k """ & cmd & " status""", 1, False
  WScript.Quit 0
End If

On Error Resume Next
rc = sh.Run(cmd & " start", 0, True)
If Err.Number <> 0 Then
  MsgBox "Failed to run start: " & Err.Description, 16, "Morning Brief"
  WScript.Quit 1
End If
On Error GoTo 0
...
If rc <> 0 Then
  ...
  sh.Run "cmd.exe /k """ & cmd & " start""", 1, False
  WScript.Quit rc
End If
