 ...
 * 精选条数上限。
 *
 * ⚠️ 原来是 12（照着北极星辅助指标的「300 条进、12 条出」定的），
 *    2026-09-25 用户反馈「最高只能改到 12，太少了」⇒ 放到 **30**：
 *    第二层只对入选的这些条目发摘要，30 条 × 约 200 token 仍在几分钱量级。
 * ⚠️ 但要如实说清楚代价：300 进 30 出 = 压缩率 **10%**，
 *    与北极星那条「≤ 5%」不再一致 —— 那是**用户自己选**的取舍，不是默认值。
 *    默认仍是 10 条（`DEFAULT_PICK_COUNT`）。
 */
